...
ggplot()+
  ...
  geom_smooth(data = tot_thick_summary, aes(x = year, y = ice_max, color = lake_id), method = 'lm', se = F)+
  facet_wrap(~lake_id)
...
ggplot()+
  geom_point(data = black_ice_summary, aes(x = year, y = black_ice_mean))+
  geom_smooth(data = black_ice_summary, aes(x = year, y = black_ice_mean, color = lake_id), method = 'lm', se = F)+
  facet_wrap(~lake_id)
...
ggplot()+
  ...
  geom_smooth(data = white_ice_summary, aes(x = year, y = white_ice_mean, color = lake_id), method = 'lm', se = F)+
  facet_wrap(~lake_id)
...
summary(tot_thick_trends %>% filter(mk_max<0.05) %>% filter(sen_max<0)) 
...
summary(white_ice_trends %>% filter(mk_mean<0.05) %>% filter(sen_mean>0)) 
...
x <- white_ice_trends %>% 
  ...
  select(lake_id, sen_wht_mean, sen_wht_max, sen_mean, sen_max) %>% 
  pivot_longer(
    !lake_id,
    names_to = 'type',
    values_to = 'trend'
...
ggplot(data = x, aes(x = type, y = trend))+
...
ggsave(here('results/ice_trend_boxplot.pdf'), dpi = 300, width = 11, height = 10, units = 'in')  

# 4. Ice thickness plot ---------------------------------------------------

trends <- tot_thick_trends %>% 
  mutate(
    mean = as.factor('mean'),
    max = as.factor('max')
  )
  
ggplot(data = tot_thick_trends)+
  geom_violin(aes(x = mean, y = sen_mean))
